...
Read `design.md` in full: character, the build mandate, composition patterns, and every rule in `## Component & usage rules` (do's and don'ts). Then read the real token values from the styling/theme system it points to: colors (light + dark), typography, spacing, radius, shadows, motion. The styling/theme system is the source of truth for values; `design.md` is the source of truth for direction. (If an older `design.md` still carries inline token values, treat the styling/theme system as authoritative where they disagree, and note it in the report.)

### DS2: Confirm token coverage

Detect the platform and its styling/theme and light/dark systems. If the page needs a token that system does not have yet, add it in the system's language (derive from the nearest existing token, follow the `design.md` rules), never a one off hardcoded value. Never silently overwrite an existing token; a genuine conflict → list it in the report, let the engineer decide.
